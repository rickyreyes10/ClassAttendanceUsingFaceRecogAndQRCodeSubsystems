...
from attendance.models import Professor, Class, AttendanceLog, AttendanceSystemRegistration, ClassEnrollment, Student
...
def professor_create_class(request):
    ...
    student_emails = data.get('studentEmails', [])
    ...
    for student_email in student_emails: 
        student, created = Student.objects.get_or_create(email=student_email) # get the student from the student table or create a new one if it doesn't exist
        ClassEnrollment.objects.create(class_crn=class_instance, student=student) # enroll the student in the class
